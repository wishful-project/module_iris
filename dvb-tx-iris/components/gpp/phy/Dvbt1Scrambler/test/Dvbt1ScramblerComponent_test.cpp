/**
 * \file components/gpp/phy/Dvbt1Scrambler/test/Dvbt1ScramblerComponent_test.cpp
 * \version 1.0
 *
 * \section COPYRIGHT
 *
 ...
 * \section DESCRIPTION
 *
 * Main test file for Dvbt1Scrambler component.
 */

#define BOOST_TEST_MODULE Dvbt1ScramblerComponent_Test
...
BOOST_AUTO_TEST_SUITE (Dvbt1ScramblerComponent_Test)

BOOST_AUTO_TEST_CASE(Dvbt1ScramblerComponent_Basic_Test)
{
  BOOST_REQUIRE_NO_THROW(Dvbt1ScramblerComponent mod("test"));
}

BOOST_AUTO_TEST_CASE(Dvbt1ScramblerComponent_Parm_Test)
{
  Dvbt1ScramblerComponent mod("test");
  BOOST_CHECK(mod.getParameterDefaultValue("debug") == "false");
}

BOOST_AUTO_TEST_CASE(Dvbt1ScramblerComponent_Ports_Test)
{
  Dvbt1ScramblerComponent mod("test");
...
BOOST_AUTO_TEST_CASE(Dvbt1ScramblerComponent_Init_Test)
{
  Dvbt1ScramblerComponent mod("test");
  mod.registerPorts();
...
BOOST_AUTO_TEST_CASE(Dvbt1ScramblerComponent_Process_Test)
{
  ...
  if(true)
  {
    ...
    BOOST_CHECK(oSet->data.size() == num); // same number as in input
    for(int i=0;i<num;i++)
    {
      if(i%1504 == 0)
        BOOST_CHECK(oSet->data[i] == 0xb8); // inverted TS sync byte
      else if(i%188 == 0)
        BOOST_CHECK(oSet->data[i] == 0x47); // TS sync byte
    }
