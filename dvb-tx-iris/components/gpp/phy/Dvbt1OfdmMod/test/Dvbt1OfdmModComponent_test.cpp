/**
 * \file components/gpp/phy/Dvbt1OfdmMod/test/Dvbt1OfdmModComponent_test.cpp
 * \version 1.0
 *
 * \section COPYRIGHT
 *
 ...
 * \section DESCRIPTION
 *
 * Main test file for Dvbt1OfdmMod component.
 */

#define BOOST_TEST_MODULE Dvbt1OfdmModComponent_test
...
BOOST_AUTO_TEST_SUITE (Dvbt1OfdmModComponent_Test)

BOOST_AUTO_TEST_CASE(Dvbt1OfdmModComponent_Basic_Test)
{
  BOOST_REQUIRE_NO_THROW(Dvbt1OfdmModComponent mod("test"));
}

BOOST_AUTO_TEST_CASE(Dvbt1OfdmModComponent_Parm_Test)
{
  ...
  BOOST_CHECK(mod.getParameterDefaultValue("outpower") == "10");
  BOOST_CHECK(mod.getParameterDefaultValue("dacsamplerate") == "0");
}

BOOST_AUTO_TEST_CASE(Dvbt1OfdmModComponent_Ports_Test)
{
...
BOOST_AUTO_TEST_CASE(Dvbt1OfdmModComponent_Init_Test)
{
  typedef std::complex<float>   Cplx;
...
BOOST_AUTO_TEST_CASE(Dvbt1OfdmModComponent_Process_Test)
{
  
  int ofdmarr[] = {2048,4096,8192};
  string ofdmstrarr[] = {"2048","4096","8192"};
  int deltaarr[] = {32,16,8,4};
  string deltastrarr[] = {"32","16","8","4"};
  ...
  for(int oi = 0; oi < sizeof(ofdmarr) / sizeof(ofdmarr[0]); oi++)
  {
    for(int di = 0; di < sizeof(deltaarr) / sizeof(deltaarr[0]); di++)
    {
      ...
      if(true)
      {
        ...
        int num = Karr[oi] * (1000000 / Karr[oi]);
        ...
        printf("%d,%d: oSet->data.size()=%lu, num=%d\n", ofdmarr[oi], deltaarr[di], oSet->data.size(), num);
        BOOST_CHECK(oSet->data.size() == (ofdmarr[oi] + ofdmarr[oi] / deltaarr[di]) * (num / Karr[oi])); // good size
      ...
      if(true)
      {
        ...
        int num2 = RawFileUtility::getNumElements<Cplx>("output" + ofdmstrarr[oi]
          + deltastrarr[di]);
        out2.getWriteData(oSet2, num2);
        RawFileUtility::read(oSet2->data.begin(), oSet2->data.end(), "output"
          + ofdmstrarr[oi] + deltastrarr[di]);
        BOOST_CHECK(oSet2->data.size() == oSet->data.size()); // good size

        // value check
        float tol = 1e-5, tol2 = tol * tol; /* 1e-5 < 2/2^16, good for regular quantization */
        for(int i=0;i<oSet2->data.size();i++)
        {
            ...
            /*if(diff2 >= tol2)
            {
              printf("%d, %d, file %d: %f, %f\n", ofdmarr[oi], deltaarr[di], i, oSet2->data[i].real(), oSet2->data[i].imag());
              printf("%d, %d, iris %d: %f, %f\n", ofdmarr[oi], deltaarr[di], i, oSet->data[i].real(), oSet->data[i].imag());
            }*/
        }
