/**
 * \file components/gpp/phy/Dvbt1ConvEncoder/benchmark/Dvbt1ConvEncoderComponent_benchmark.cpp
 * \version 0.1
 *
 * \section COPYRIGHT
 *
 ...
 * \section DESCRIPTION
 *
 * Main benchmark file for Dvbt1ConvEncoder component.
 */
...
int main(int argc, char* argv[])
{
  ...
  int num = 204*(10000000/204); // bytes
  ...
  float bitsPerSec = ((8*num)/(1.0e-9 * time.total_nanoseconds()));
  cout << "Rate = " << bitsPerSec*1.0e-6 << " Mb/s" << endl;

  float maxRate = 31.67e6 * 204 / 188; // table A.1 pag. 40
  cout << "Maximum R-S encoded bit rate in ETSI EN 300 744 is " << maxRate*1.0e-6 << " Mb/s" << endl;
